(function(){
  ...
  function send(action,target){
    db.from('usage_events').insert({page,page_title:title,action:action||'view',target:target||null,session_id:visitorId,metadata:{referrer:document.referrer?new URL(document.referrer).pathname:null}}).then(()=>{}).catch(()=>{});
  }
  ...
  document.addEventListener('click',function(e){
    ...
    if(a){
      ...
      if(href && !href.startsWith('#') && !href.startsWith('javascript:')){
        let target=href;
        try{target=new URL(href,location.href).pathname.split('/').pop()||'index.html'}catch(_){target=href}
        send('click',target);
      }
    }
